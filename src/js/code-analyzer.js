...
function jsonToModelConds(parsecode ){
    ...
    case 'ForStatement' : models.push({line : parsecode.loc.start.line , 'type' : 'ForStatement' , 'name' : '' ,'condition' : get_expression(parsecode.init) + ';' + create_condition(parsecode.test) + ';' + get_expression(parsecode.update)});models.push(jsonToModel(parsecode.body ) ); break;
...
function handle_exprssion_statement(parsecode){
    ...
    {
        models.push({'line' :parsecode.expression.loc.start.line , 'type' : parsecode.expression.type , 'name' :parsecode.expression.left.name , 'condition' : '' , 'value' : get_expression(parsecode.expression.right)} );
    }
    else
    {
...
function get_complex_expprsion(right){
    ...
    case 'MemberExpression' : res = res + right.object.name +'[' + get_expression(right.property) + ']' ; break;
